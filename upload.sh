...
OF_PUTURL=$(curl -X POST --data '{"filename":"plugin.xpi"}' https://open.ge.tt/1/files/63sezAX/create?accesstoken=$OF_ACCESSTOKEN 2> /dev/null | python -c 'import json,sys;obj=json.load(sys.stdin);print obj["upload"]["puturl"]')
